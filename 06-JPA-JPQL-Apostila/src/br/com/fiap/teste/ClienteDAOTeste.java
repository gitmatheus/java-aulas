...
class ClienteDAOTeste {
	...
	void pesquisarPorNomeCidadeTest() {
		List<Cliente> lista = dao.pesquisar("Lea","Lon");
		
		for(Cliente cliente : lista) {
			assertTrue(cliente.getNome().contains("Lea") && cliente.getEndereco()
					.getCidade().getNome().contains("Lon"));
		}
	...
	void pesquisarPorEstado() {
		...
		List<Cliente> lista = dao.perquisarPorEstados(estados);
		for(Cliente cliente : lista) {
